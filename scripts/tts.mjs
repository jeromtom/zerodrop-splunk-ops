...
const beats = [
  ...
  ["03-agent-short", "DropWatch isn't a passive dashboard, it's an agent. It pulls telemetry from Splunk over the M-C-P server and reasons with a language model. The verdict: a high-severity security finding. One subnet, ten-dot-sixty-six-dot-six-dot-zero slash twenty-four, drives most of the oversell-rejects, so DropWatch flags it as automated scalping, OWASP O-A-T zero-zero-five, and recommends blocking it. And it monitors its own agent: latency, token cost, confidence, and drift, shipped back to Splunk. No oversell ever happened. The guard held, a hundred out of a hundred."],
  ["04-apply", "It doesn't just alert, it acts. One click blocks the subnet, ten-dot-sixty-six-dot-six-dot-zero slash twenty-four, and the action is written straight back to Splunk, so the remediation itself is observable. The loop closes: pull, reason, recommend, act."],
  ["05-close", "DropWatch turns every hot path into Splunk telemetry over H-E-C, and an agent that pulls it back over the M-C-P server, reasons about it, acts, and monitors its own reasoning. Oversell-proof by construction, and now fully observable, on Splunk."],
