...
const beats = [
  ...
  ["03-agent-short", "DropWatch isn't a passive dashboard — it's an agent. It reads the live telemetry stream, and a real language model reasons about drop health. The header confirms it: reasoning on the live A-I-M-L model. The verdict: drop-health, seventy-five out of a hundred, with a high-severity finding — oversell-bot activity. One subnet, ten-dot-sixty-six-dot-six-dot-zero slash twenty-four, produces most of the oversell-rejects. That's the bot signature. And no oversell ever happened — the DynamoDB guard held. Meanwhile, claim rate runs about seven times baseline. That's the stampede."],
